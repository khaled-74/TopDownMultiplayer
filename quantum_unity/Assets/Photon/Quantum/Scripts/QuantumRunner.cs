...
public sealed class QuantumRunner : MonoBehaviour, IDisposable {
  ...
  public struct StartParameters {
    ...
    /// <summary>
    /// The replay provider injects recorded inputs and rpcs into the game which is required to run the game as a replay. <see cref="InputProvider"/> is an implementation of the replay provider. See useages of <see cref="QuantumGame.RecordedInputs"/> and <see cref="QuantumRunnerLocalReplay._inputProvider"/>.
    /// </summary>
  ...
  public QuantumProfilingClient ProfilingClient;
#endif
...
#if QUANTUM_REMOTE_PROFILER
    if (ProfilingClient != null) {
      ProfilingClient.Dispose();
      ProfilingClient = null;
    }
#endif
  }
  ...
  public static QuantumRunner StartGame(String clientId, StartParameters param) {
    ...
    if (!Application.isEditor) {
      runner.ProfilingClient = new QuantumProfilingClient(clientId, deterministicConfig, info);
      runner.Game.ProfilerSampleGenerated += (sample) => {
        runner.ProfilingClient.SendProfilingData(sample);
        runner.ProfilingClient.Update();
      };
    }
#endif
  ...
  public static DeterministicPlatformInfo CreatePlatformInfo() {
    ...
    // WebGL does not support multithreading. This forces the simulation to run in single-thread.
